...
function debugNode(node) {
  ...
  if (typeof node === 'string' || typeof node === 'number') return (0, _underscore.escape)(node);
  if (!node) return '';
...
function debugInst(inst) {
  ...
  if (typeof inst === 'string' || typeof inst === 'number') return (0, _underscore.escape)(inst);
  if (!inst) return '';

  if (!inst.getPublicInstance) {
    var internal = (0, _MountedTraversal.internalInstance)(inst);
    return debugInst(internal, indentLength);
  }

  var publicInst = inst.getPublicInstance();

  if (typeof publicInst === 'string' || typeof publicInst === 'number') return (0, _underscore.escape)(publicInst);
  ...
  var childrenStrs = (0, _underscore.compact)(children.map(function (n) {
